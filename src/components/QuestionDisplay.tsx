...
export const QuestionDisplay = memo<QuestionDisplayProps>(({
  ...
  // Auto-detect if this is a Div 2 (numerical) question - should not show MCQ options
  const isDiv2Question = question.division_override === 2 ||
    (question.division_override !== 1 && !['A', 'B', 'C', 'D'].includes((question.answer || '').toUpperCase().trim()));
  const shouldHideOptions = hideOptions || isDiv2Question;
