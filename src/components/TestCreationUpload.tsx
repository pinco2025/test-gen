...
interface TestCreationUploadProps {
  ...
  onAutoSelect?: (data: FullTestJson, presetId: string) => void; // New callback for auto-selection
  testType: TestType;
}
...
const TestCreationUpload: React.FC<TestCreationUploadProps> = ({ onCancel, onProceed, onAutoSelect, testType }) => {
  ...
  const [showAutoSelectMode, setShowAutoSelectMode] = useState<boolean>(false);
        ...
        <div className="flex items-center gap-3">
          ...
          {presets.length > 0 && onAutoSelect && (
            ...
            >
              <span className="material-symbols-outlined text-lg">
                {showAutoSelectMode ? 'auto_awesome' : 'tune'}
              </span>
              {showAutoSelectMode ? 'Auto Mode' : 'Auto-Select'}
            </button>
          )}
          ...
          {!showAutoSelectMode && (
            <button
              ...
              disabled={!isValid}
              className={`px-6 py-2 rounded-lg font-bold flex items-center gap-2 transition-all ${isValid
        ...
        <div className="flex gap-2">
          {parsedData.tests.map((test, idx) => (
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-all ${activeTestIndex === idx
