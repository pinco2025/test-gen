...
export const DatabaseCleaning: React.FC<DatabaseCleaningProps> = ({
  ...
  const [examType, setExamType] = useState<ExamType>(() => {
    const stored = localStorage.getItem('databaseCleaning_examType');
  ...
  const handleIPQSave = async (newQuestion: Question, newSolution?: Partial<Solution>) => {
    ...
    try {
      // Determine parent exam from current examType
      ...
      addNotification('success', `IPQ created successfully!`);
  ...
  }), [filteredQuestions, handleEdit, handleCreateIPQ, setSize, zoomLevel, isSelectionMode, selectedUuids, toggleQuestionSelection]);
        ...
        <div className="flex gap-8">
          {['Physics', 'Chemistry', 'Mathematics'].map((subject) => (
            <button
              ...
              className={`py-4 font-semibold text-lg border-b-2 transition-all ${activeSection === subject
          ...
          <div className="bg-white dark:bg-[#1e1e2d] p-4 rounded-xl border border-gray-200 dark:border-[#2d2d3b] shadow-sm flex flex-col h-full overflow-hidden">
            <div className="flex-shrink-0 mb-4 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-gray-500 dark:text-gray-400 ml-2">
                  {loading ? 'Loading...' : `${availableQuestions.length} questions`}
                </span>
              </div>
            </div>
            ...
            <div className="flex-shrink-0 flex gap-4 mb-4">
              ...
              <button
                ...
                className={`px-4 py-2 rounded-full border transition-all flex items-center gap-2 font-medium ${isSelectionMode
