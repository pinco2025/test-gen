  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // We need to fetch solutions separately as getByUUIDs (questions table) doesn't include solution data
        // Assuming there is an API or we need to request it.
        // Checking available APIs... questions.getSolution is available.

        let originalSol: Solution | null = null;
        let currentSol: Solution | null = null;

        try {
            originalSol = await window.electronAPI.questions.getSolution(originalQuestionUuid);
        } catch (e) { console.warn("Failed to fetch original solution", e); }

        try {
            currentSol = await window.electronAPI.questions.getSolution(currentQuestion.uuid);
                ...
                <>
                    ...
                    <div className="flex-1 flex flex-col bg-white dark:bg-[#1e1e2d]">
                         <div className="p-3 bg-blue-50 dark:bg-blue-900/10 border-b border-blue-100 dark:border-blue-900/20 flex justify-between items-center">
                            <span className="font-bold text-sm text-blue-700 dark:text-blue-400 uppercase tracking-wider">New IPQ Question</span>
